...
var require_coerce = __commonJS({
  "../../node_modules/semver/functions/coerce.js"(exports, module2) {
    ...
    var coerce = (version2, options) => {
      ...
      return parse5(`${major}.${minor}.${patch}${prerelease}${build3}`, options);
    };
  ...
  "../../node_modules/semver/classes/range.js"(exports, module2) {
    ...
    var Range = class {
      constructor(range, options) {
        ...
        this.set = this.raw.split("||").map((r2) => this.parseRange(r2.trim())).filter((c5) => c5.length);
        if (!this.set.length) {
...
async function readFilesAndDirectories(basePath) {
  try {
...
async function buildWorkerFile({ vercelConfig, vercelOutput }, {
  outputDir,
  workerJsDir,
  nopDistDir,
  templatesDir,
...
async function buildNextStaticHeaders(outputDir, opts) {
...
async function collectFunctionConfigsRecursively(baseDir, configs = {
  ...
  const dirs = paths.filter((path2) => path2.isDirectory);
  const files = paths.filter((path2) => !path2.isDirectory);
  for (const { path: path2 } of dirs) {
    if (path2.endsWith(".func")) {
      ...
      const isPrerenderedIsrFunc = config?.operationType?.toLowerCase() === "isr" && !path2.endsWith(".action.func");
      const isPrerenderedApiFunc = config?.operationType?.toLowerCase() === "api" && checkPrerenderConfigExists(path2, files);
...
function checkPrerenderConfigExists(funcPath, files) {
  const prerenderConfigPath = funcPath.replace(
    /\.func$/,
    ".prerender-config.json"
  );
  return files.find(({ path: path2 }) => path2 === prerenderConfigPath);
}
...
async function processPrerenderFunctions({
  prerenderedFunctions,
  invalidFunctions,
  ignoredFunctions
}, opts) {
  for (const [path2, fnInfo] of prerenderedFunctions) {
    ...
    if (routeInfo) {
      ...
      const rscVariantPath = path2.replace(/\.func$/, ".rsc.func");
...
function extractWebpack(statement) {
  if (statement.type !== "ExpressionStatement")
    return [];
  const expr = statement.expression.type === "SequenceExpression" ? statement.expression.expressions.find(
    (expr2) => expr2.type === "CallExpression"
  ) : statement.expression;
  if (expr?.type !== "CallExpression" || expr.callee.type !== "MemberExpression" || expr.callee.property.type !== "Identifier" || expr.callee.property.name !== "push" || expr.callee.object.type !== "AssignmentExpression" || !isSelfWebpackChunk_N_E(expr.callee.object.left) || expr.callee.object.right.type !== "LogicalExpression" || !isSelfWebpackChunk_N_E(expr.callee.object.right.left) || expr.callee.object.right.right.type !== "ArrayExpression" || expr.callee.object.right.right.elements.length !== 0 || expr.arguments[0]?.type !== "ArrayExpression" || expr.arguments[0].elements[1]?.type !== "ObjectExpression") {
    return [];
  }
...
async function buildFile(contents, filePath, { relativeTo } = {}) {
  ...
  const nextBuildID = await (0, import_promises8.readFile)((0, import_node_path7.resolve)(".next", "BUILD_ID"), "utf8");
...
async function processFunctionIdentifiers({ edgeFunctions }, { entrypointsMap, identifierMaps }, opts) {
  ...
  for (const [path2, fnInfo] of edgeFunctions) {
    ...
    for (const { type, identifier: identifier2, start, end, importPath } of identifiers) {
      ...
      if (importPath) {
        const { updatedContents, newImportToPrepend } = await processImportIdentifier(
    ...
    await Promise.all(
      [...identifierPathsToBuild].map(async (path3) => {
        const fileContents2 = await functionifyFileContent(path3);
        return buildFile(fileContents2, path3);
      })
    );
...
async function functionifyFileContent(path2) {
  const originalFileContents = await (0, import_promises9.readFile)(path2, "utf8");
  return `
		${""}
...
async function buildFunctionFile({ fnInfo, fileContents, newFnLocation, newFnPath }, { importsToPrepend }, { workerJsDir, nopDistDir }) {
  ...
  let chunkMapIdx = 0;
  const chunksExportsMap = /* @__PURE__ */ new Map();
  const relativeImportPath = getRelativePathToAncestor({
    from: newFnLocation,
    relativeTo: nopDistDir
  });
  groupedImports.forEach((exports, path2) => {
    ...
    if (path2.endsWith(".wasm")) {
      ...
      functionImports.push(`import ${defaultExport} from '${path2}';`);
      return;
    }
...
function iifefyFunctionFile(fileContents, functionImports, functionInfo, chunksExportsMap) {
  const fileContentsContainEntriesDeclaration = /(let|var|const)\s+_ENTRIES\s*=/.test(fileContents);
  ...
  const wrappedContent = `
		export default ((self, globalThis, global) => {
			${fileContents.replace(/export\s+default\s+/g, "return ")}
		})(proxy, proxy, proxy);
	`;
  const proxyCall = `const proxy = globalThis.__nextOnPagesRoutesIsolation.getProxyFor('${functionInfo.route?.path ?? ""}');`;
    ...
    [...wasmImportsToPrepend.entries()].map(
      async ([codeBlockRelativePath, wasmImports]) => {
        ...
        for (const identifier2 of wasmImports) {
          ...
          functionImports.push(`import ${identifier2} from "${wasmImportPath}"`);
        }
...
async function processImportIdentifier(ident, {
  fileContents,
  entrypoint,
  newFnLocation,
  fnConfig
}, { nopDistDir, workerJsDir }) {
  const { type, identifier: identifier2, start, end, importPath, info } = ident;
  let updatedContents = fileContents;
  const codeBlock = updatedContents.slice(start, end);
  if (!info.newDest) {
    const importPathWithoutLeadingDots = importPath.replace(/^(\.\.\/)+/, "");
    const importPathWithoutType = importPathWithoutLeadingDots.replace(new RegExp(`^/${type}/`), "/");
    const oldPath = (0, import_node_path8.join)((0, import_node_path8.dirname)(entrypoint), importPath);
    const originalFilePath = (0, import_node_path8.resolve)(
      fnConfig.filePathMap?.[importPathWithoutLeadingDots.replace(/^\//, "")] ?? oldPath
    );
...
async function processBundledAssets({ edgeFunctions }, { nopDistDir }) {
  for (const [functionPath, { relativePath, config }] of edgeFunctions) {
    for (const { name, path: path2 } of config.assets ?? []) {
      const originalFile = (0, import_node_path8.resolve)(
        config.filePathMap?.[path2] ?? (0, import_node_path8.join)(functionPath, path2)
      );
...
var namedExportsObjectName = "__namedExportsObject";
var getNamedExportsFunctionName = "__getNamedExports";
